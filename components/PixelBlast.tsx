...
const PixelBlast: React.FC<PixelBlastProps> = ({
  variant = 'square',
  pixelSize = 3,
  color = '#0a3d3d', // Dark cyan - subtle background that doesn't conflict with text/buttons
  className,
  style,
  antialias = true,
  patternScale = 2,
      ...
      const animate = () => {
        ...
        if (composer) {
          ...
          composer.passes.forEach((p: Pass) => {
            const pass = p as unknown as { effects?: Array<Effect & { uniforms: Map<string, THREE.Uniform> }> };
            if (pass.effects) {
